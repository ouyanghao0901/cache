<template>
  <div class="goods">
    <!-- 分类列表 -->
    <div class="menu-wrapper" ref="menuScroll">
      <ul>
        <!-- 专场 -->
        <li :class="{'currentIndex':currentIndex===0}" @click="selectMenu(0)" class="menu-item">
          <p class="text">
            <img class="icon" :src="container.tag_icon" v-if="container.tag_icon">
            {{container.tag_name}}
          </p>
        </li>
        <li
          class="menu-item"
          v-for="(item,index) in goods"
          :class="{'currentIndex':currentIndex===index+1}"
          :key="index"
          @click="selectMenu(index+1)"
        >
          <p class="text">
            <img class="icon" :src="item.icon" v-if="item.icon">
            {{item.name}}
          </p>
          <i v-show="calculateCount(item.spus)" class="num">{{calculateCount(item.spus)}}</i>
        </li>
      </ul>
    </div>
    <!-- 商品列表 -->
    <div class="foods-wrapper" ref="foodScroll">
      <ul>
        <!-- 专场 -->
        <li class="container-list food-list-hook">
          <div v-for="(item,index) in container.operation_source_list" :key="index">
            <img :src="item.pic_url">
          </div>
        </li>
        <!-- 具体分类 -->
        <li v-for="(item,index) in goods" :key="index" class="food-list food-list-hook">
          <h3 class="title">{{item.name}}</h3>

          <!-- 具体商品列表 -->
          <ul>
            <li
              class="food-item"
              v-for="(food,index) in item.spus"
              :key="index"
              @click="showDetail(food)"
            >
              <div class="icon" :style="head_bg(food.picture)"></div>
              <div class="content">
                <h3 class="name">{{food.name}}</h3>
                <p v-if="food.description" class="desc">{{food.description}}</p>
                <div class="extra">
                  <span class="saled">{{food.month_saled_content}}</span>
                  <span class="praise">{{food.praise_content}}</span>
                </div>
                <img class="product" :src="food.product_label_picture">
                <p class="price">
                  <span class="text">¥{{food.min_price}}</span>
                  <span class="unit">/{{food.unit}}</span>
                </p>
              </div>
              <div class="cartcontrol-wrapper">
                <cartcontrol :food="food"></cartcontrol>
              </div>
            </li>
          </ul>
        </li>
      </ul>
    </div>
    <!-- 购物车 -->
    <shopcart :poiInfo="poiInfo" :selectFoods="selectFoods"></shopcart>

    <!-- 商品详情 -->
    <productdetail :food="selectFood" ref="foodView"></productdetail>
  </div>
</template>

<script>
import BScroll from "better-scroll";
import shopcart from "@/components/shopcart/Shopcart";
import cartcontrol from "@/components/cartcontrol/CartControl";
import productdetail from "@/components/productDetail/productdetail";

export default {
  data() {
    return {
      container: {},
      goods: [],
      listHeight: [],
      menuScroll: {},
      foodScroll: {},
      scrollY: 0,
      poiInfo: {},
      selectFood: {}
    };
  },
  methods: {
    head_bg(imgname) {
      return "background-image:url(" + imgname + ")";
    },
    initScroll() {
      this.menuScroll = new BScroll(this.$refs.menuScroll, {
        click: true
      });
      this.foodScroll = new BScroll(this.$refs.foodScroll, {
        probeType: 3,
        click: true
      });
      // 监听事件
      this.foodScroll.on("scroll", pos => {
        this.scrollY = Math.abs(Math.round(pos.y));
      });
    },
    calculateHeight() {
      //获取元素
      let foodList = this.$refs.foodScroll.getElementsByClassName(
        "food-list-hook"
      );
      let height = 0;
      this.listHeight.push(height);
      for (let i = 0; i < foodList.length; i++) {
        let item = foodList[i];
        height += item.clientHeight;
        this.listHeight.push(height);
      }
    },
    selectMenu(index) {
      let foodList = this.$refs.foodScroll.getElementsByClassName(
        "food-list-hook"
      );
      let ele = foodList[index];
      //滚动到对应位子
      this.foodScroll.scrollToElement(ele, 200);
    },
    calculateCount(spus) {
      let count = 0;
      spus.forEach(item => {
        if (item.count > 0) {
          count += item.count;
        }
      });
      return count;
    },
    showDetail(food) {
      this.selectFood = food;

      this.$refs.foodView.showView();
    }
  },
  created() {
    fetch("/api/goods")
      .then(resp => resp.json())
      .then(resp => {
        if (resp.code == 0)
          this.container = resp.data.container_operation_source;
        this.goods = resp.data.food_spu_tags;
        this.poiInfo = resp.data.poi_info;
        //dom 已经更新才会执行这个方法
        this.$nextTick(() => {
          //执行滚动方法
          this.initScroll();
          //计算区间高度
          this.calculateHeight();
        });
      });
  },
  computed: {
    currentIndex() {
      for (let i = 0; i < this.listHeight.length; i++) {
        // 获取商品区间的范围
        let height1 = this.listHeight[i];
        let height2 = this.listHeight[i + 1];
        //是否在上述区间
        if (!height2 || (this.scrollY >= height1 && this.scrollY <= height2)) {
          return i;
        }
      }
      return 0;
    },

    selectFoods() {
      let foods = [];
      this.goods.forEach(myFoods => {
        myFoods.spus.forEach(food => {
          if (food.count > 0) foods.push(food);
        });
      });
      return foods;
    }
  },
  components: {
    shopcart,
    cartcontrol,
    productdetail
  }
};
</script>

<style lang="scss" scoped>
.currentIndex {
  background-color: white;
}
.goods {
  width: 100%;
  display: flex;
  position: absolute;
  top: 170px;
  bottom: 51px;
  overflow: hidden;
  > .menu-wrapper {
    flex: 0 0 85px;
    background-color: #f4f4f4;
    > ul {
      > .menu-item {
        padding: 16px 23px 15px 10px;
        border-bottom: 1px solid #e4e4e4;
        position: relative;
        > .text {
          font-size: 13px;
          color: #333333;
          line-height: 17px;
          vertical-align: middle;

          -webkit-line-clamp: 2;
          display: -webkit-box;
          -webkit-box-orient: vertical;
          overflow: hidden;
          > .icon {
            width: 15px;
            height: 15px;
            vertical-align: middle;
          }
        }
      }
    }
  }
  > .foods-wrapper {
    flex: 1;
    // overflow: auto;
  }
}

.foods-wrapper .container-list {
  padding: 11px 11px 0 11px;
  border-bottom: 1px solid #e4e4e4;
}
.foods-wrapper .container-list img {
  width: 100%;
  margin-bottom: 11px;
  border-radius: 5px;
}
// 具体分类商品布局
.foods-wrapper .food-list {
  padding: 11px;
}
.foods-wrapper .food-list .title {
  height: 13px;
  font-size: 13px;
  background: url(./img/2x.png) no-repeat left center;
  background-size: 2px 10px;
  padding-left: 7px;
  margin-bottom: 12px;
}
.foods-wrapper .food-list .food-item {
  display: flex;
  margin-bottom: 25px;
  position: relative;
}

.foods-wrapper .food-list .food-item .icon {
  flex: 0 0 63px;
  background-position: center;
  background-size: 120% 120%;
  background-repeat: no-repeat;
  margin-right: 11px;
  height: 75px;
}
.foods-wrapper .food-list .food-item .content .name {
  font-size: 16px;
  line-height: 21px;
  color: #333333;
  margin-bottom: 10px;
  padding-right: 27px;
}
.foods-wrapper .food-list .food-item .content .desc {
  font-size: 10px;
  line-height: 19px;
  color: #bfbfbf;
  margin-bottom: 8px;

  // 超出部分显示省略号
  -webkit-line-clamp: 1;
  display: -webkit-box;
  -webkit-box-orient: vertical;
  overflow: hidden;
}
.foods-wrapper .food-list .food-item .content .extra {
  font-size: 10px;
  color: #bfbfbf;
  margin-bottom: 7px;
}
.foods-wrapper .food-list .food-item .content .extra .saled {
  margin-right: 14px;
}
.foods-wrapper .food-list .food-item .content .product {
  height: 15px;
  margin-bottom: 6px;
}
.foods-wrapper .food-list .food-item .content .price {
  font-size: 0;
}
.foods-wrapper .food-list .food-item .content .price .text {
  font-size: 14px;
  color: #bf4e44;
}
.foods-wrapper .food-list .food-item .content .price .unit {
  color: #bfbfbf;
  font-size: 12px;
}

.foods-wrapper .food-list .food-item .cartcontrol-wrapper {
  position: absolute;
  right: 0;
  bottom: 0;
}

.goods .menu-wrapper .menu-item .num {
  position: absolute;
  right: 5px;
  top: 5px;
  width: 13px;
  height: 13px;
  color: white;
  border-radius: 50%;
  background-color: red;
  text-align: center;
  font-size: 7px;
  line-height: 13px;
}
</style>

